...
## 最新更新：v0.9.3 审批与重试可靠性修复

- **保存失败不会自动执行**：审批和重试保存期间，任务不能进入调度；保存失败会恢复原状态，不增加重试次数或留下未确认的批准事件。
- **中断后恢复原决定**：保存中的审批或重试带有恢复记录。服务重启时恢复操作前的状态，显示中文提示，等待重新确认；已有用量不回退。
- **重试清理忽略文件**：重置实施工作区时也清理被 Git 忽略的未跟踪文件，避免读取上次尝试的缓存或临时配置。重置前核对任务路径、真实路径与 Git 根目录；路径被重定向时拒绝重置。

先退出旧服务，运行 `git pull --ff-only` 和 `npm run open`，再刷新页面。没有新增依赖或手工数据迁移。本次是源码更新，未发布安装包；验证使用模拟 CLI，不包含真实账号执行或原生桌面验收。

149 项本地 Node 测试全部通过，44 个运行及浏览器脚本的语法与模块引用检查通过。

## v0.9.2 用量恢复与后台开销优化

- **恢复后预算保持准确**：如果异常退出前账本比任务快照更新，重启会先恢复任务累计 Token、耗时和模型信息，再进行审批或重试预算检查；重复重启不会重复累计已保存的耗时。
- **减少无效汇总与推送**：用量总计和模型统计随用量变化增量更新。普通日志不再触发全量历史用量扫描；用量或历史成员变化时才推送汇总，没有实时订阅者时跳过这项广播。
- **减少账本写盘**：只有账本变化后才保存。保存进行中又收到新用量时，后续请求仍会等待覆盖新数据的保存；失败后仍可重试。
- **启动时也可退出**：关闭服务会先取消启动恢复中的 Git 操作，避免等到 Git 超时才退出，并保留尚未核对完成的应用记录。

退出旧服务后运行 `git pull --ff-only` 和 `npm run open`，再刷新页面。没有新增依赖或需要手工执行的数据迁移。本次为源码更新，未发布桌面安装包或调用真实模型。

140 项本地 Node 测试全部通过，44 个运行及浏览器脚本的语法与模块引用检查通过。新增回归覆盖账本领先的重启恢复、1,000 条历史任务下的实时日志，以及启动阶段的 Git 取消。

## v0.9.1 故障恢复与配额修复

- **取消任务时清理工具进程**：Codex 和 Git 使用统一的进程清理机制。macOS / Linux 清理独立进程组，Windows 使用系统 `taskkill /T`；等待清理后才释放并发名额和工作区。
- **应用操作可恢复**：写入原仓库前保存审批意图和结果指纹。若随后保存任务记录失败，重启会核对 Git 索引与工作区；已应用的结果直接恢复为完成，不重复应用。存在额外修改时保留文件，显示“核对应用结果”。
- **删除历史不返还配额**：独立用量账本保留累计 Token；某任务报告仓库已超限时，同仓库其他运行或排队任务也会停止。用量概览包含已删除历史的累计统计。
- **数据目录独占**：同一数据目录只允许一个新版执行器写入，即使端口不同。正常退出释放锁；确认旧进程已退出后可回收其锁。无法验证锁的归属时拒绝启动。

先在所有旧服务的启动终端按 **Ctrl+C** 正常退出，再更新：
...
刷新页面，有更新提示时点击“重新加载”。无需新增依赖；账本会自动从现有任务记录初始化，升级前已经删除的用量无法追溯恢复。详情见 [中文使用指南](docs/GETTING_STARTED.zh-CN.md)。本次为源码更新，未发布安装包或运行真实账号任务。

134 项本地 Node 测试全部通过，44 个脚本的语法与模块引用检查通过。覆盖真实临时 Git 仓库、模拟 CLI、进程树清理、写盘失败、重启恢复和并发配额；不包含原生桌面界面验收。

## v0.9.0 后端稳定性优化

- **减少重复写盘**：运行快照采用一个正在保存的批次和一个后续批次，并发更新合并后再序列化。保存失败会返回错误，后续保存仍可恢复。
- **保存与状态一致**：项目和模板先写盘，再更新内存与页面；同一集合的修改依次处理。新任务保存成功后才能执行，创建中断的记录重启后需要检查并手动重试。创建 worktree 前先保存恢复所需的基线和路径。
- **实时推送有边界**：慢连接的同一任务更新会合并；超时或积压过多的连接断开后可自动重连。最多允许 32 个实时连接，无订阅者时跳过事件序列化。
- **Git 操作可控**：默认 120 秒超时，标准输出最多 16 MiB，诊断保留末尾 64 KiB。超长输出不会成为不完整的审批补丁；取消任务也能停止 Git 准备阶段。退出会等待请求、清理和最终保存。
- **运行状态更完整**：健康接口增加排队任务、正在处理的任务、Git 命令和实时连接数；新增错误提示提供中文说明。

先在原启动终端按 **Ctrl+C** 退出服务，再更新并启动：
...
刷新页面；有更新提示时点击“重新加载”。无需新增依赖或手工迁移数据。大型仓库可按 [中文使用指南](docs/GETTING_STARTED.zh-CN.md) 调整 Git 超时时间。

119 项本地 Node 测试全部通过，40 个脚本的语法与模块引用检查通过。验证覆盖本地 Git、模拟 CLI、写盘失败、Git 准备阶段取消和实时推送流控。本次提供源码更新，未生成安装包；不包含真实账号任务或 Mac 原生界面验收。

## v0.8.0 前端工作台优化

- **分区导航**：任务、项目、用量、环境、设置和“登录与使用”分别显示，支持地址栏定位与浏览器前进/后退。切换分区保留表单输入和运行数据，首页提供简洁的准备状态提示。
- **任务更易扫读**：调整卡片层次与状态标识，增加失败任务入口和“舒适 / 紧凑”列表密度；密度选择保存在当前浏览器。
- **报告阅读模式**：常用 Markdown 标题、列表、表格、引用和代码块可直接阅读，也可切回原文。复制与下载始终使用当前执行的完整原文；原始 HTML 不会执行，图片不会自动加载。
- **按文件查看补丁**：文件搜索、增删统计、旧/新行号和自动换行。每次最多增加 500 行显示内容，减少大补丁的初始渲染量。文件筛选只改变查看范围，审批和导出仍包含整个补丁。
- **详情与小屏交互**：固定详情标签和操作区，中间内容独立滚动；窄屏使用全屏详情和上下排列的文件列表。键盘焦点、执行记录与阅读位置继续保留。

在原启动终端按 **Ctrl+C** 正常退出服务，再运行：
...
刷新页面；出现更新提示时点击“重新加载”。本次没有新增依赖或数据迁移，浏览器模式无需运行 `npm install`。登录入口在左侧或顶部导航的“登录与使用”。操作步骤见 [中文使用指南](docs/GETTING_STARTED.zh-CN.md)。

107 项本地 Node 测试全部通过，脚本语法、HTML/ARIA 引用、静态选择器和资源引用检查通过；新页面资源已通过本地 HTTP 服务逐一核对。本次为源码更新，未生成安装包。交互验证使用前端控制器及本地 HTTP/Git 测试；本环境无法完成真实浏览器布局和 Mac 原生交互验收。

## v0.7.0 执行记录与审批检查

- **运行状态更准确**：任务卡片和详情页每秒更新执行耗时；停止清理和长时间没有新事件时有明确说明，不会仅因没有输出就把任务判为失败。任务响应增加版本与 Runner 实例标识，防止延迟回复覆盖新状态。
- **分次保留报告**：“输出”可切换分析、实施和重试记录。失败、取消和预算停止也会保存已有报告与诊断；复制和下载对应当前选择。旧版输出仍可阅读。
- **运行中保存**：每 5 秒保存一次正在执行的记录。重启后保留最近保存的报告，并标记意外中断；耗时只计到最后保存点，不累计离线时间。需要检查后手动重试，工作区恢复仍遵守原有审批规则。
- **审批更直观**：“代码变更”增加“检查是否可应用”，检查原仓库基线和补丁适用性，不修改文件。正式应用时会再次检查；检查结果不等于审批。
- **异常处理**：单条异常 JSON 或超长事件不会打断后续报告和用量解析。修复只有心跳而未收到任务快照时无法恢复的问题；普通快照更新不会反复触发登录和环境检查。

在原启动终端按 **Ctrl+C** 正常退出服务，然后更新：
...
刷新页面，有更新提示时点击“重新加载”。无需新增依赖或手工迁移数据。保留最近 20 次执行记录；新报告最多保留前 262,144 个字符，诊断保留末尾 32,768 个字符，截断会在界面标注。异常退出时最近一次保存之后的内容可能丢失。详细操作见 [中文使用指南](docs/GETTING_STARTED.zh-CN.md)。

98 项本地 Node 测试全部通过，35 个脚本的语法、HTML 结构与 ARIA 引用、页面选择器和资源引用检查通过。验证覆盖本地 HTTP/Git 流程、运行中保存、重启恢复及前端控制器。本次提供源码更新；真实浏览器布局、Mac 原生交互和真实 Codex 账号任务仍需在本机验证，未生成新的安装包。
