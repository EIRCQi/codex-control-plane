...
export function setupTaskComposer({request, onCreated, onNeedProject, escapeHtml, notify}) {
  ...
  function remember() {
    saved = values();
    try { localStorage.setItem(draftKey, JSON.stringify(saved)); status.textContent = 'Draft saved in this browser'; }
    catch { status.textContent = 'Draft could not be saved. Keep this tab open.'; }
  }
  function sync() {
    ...
    document.querySelector('#task-policy-title').textContent = review ? 'Read-only review' : 'Protected execution';
    document.querySelector('#task-policy-text').textContent = review ? 'Codex reviews an isolated snapshot and returns a report. This task cannot request write access.' : 'First review the analysis, then approve implementation. Applying the final diff needs your approval too.';
    document.querySelector('#task-template-note').textContent = template?.description || 'Start with your own instructions, or choose a reusable template.';
    document.querySelector('#task-preview').textContent = previewTask(template, form.elements.prompt.value) || 'Write task details to preview the instructions.';
    submit.textContent = busy ? 'Creating task…' : review ? 'Start review' : 'Start analysis';
  ...
  function catalogChanged(nextProjects, nextTemplates) {
    ...
    form.elements.projectId.innerHTML = '<option value="">Choose project…</option>' + projects.map((item) => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)} · ${escapeHtml(item.branch || '')}</option>`).join('');
    form.elements.templateId.innerHTML = '<option value="">No template</option>' + templates.map((item) => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join('');
    ...
    if (!loaded) {
      ...
      if (selected.prompt) status.textContent = 'Saved draft restored';
    }
    ...
    if (missing) error.textContent = 'A saved project or template is no longer available. Check your selections before starting.';
    loaded = true; sync();
  }
  ...
  document.querySelector('#reset-task').addEventListener('click', () => {
    ...
    try { localStorage.removeItem(draftKey); status.textContent = 'Draft cleared'; }
    catch { status.textContent = 'Unable to clear the stored draft'; }
  ...
  return {
    open() {
      if (!connected) { notify('Reconnect to the runner before creating a task', {kind:'error'}); return; }
